...
const VideoPlayer = ({ src, poster, autoPlay = false, className = "", isPreview = false }) => {
    ...
    return (
        ...
        >
            ...
            {/* Always show if paused. If playing, show only if hovering? No, typical behavior: click video to pause/play, big icon animation or nothing */}
            {/* User Request: "just a play button at middle is center is enough" (for preview) */}

            {(!isPlaying || (showControls && isFullUI)) && (
                <div
                    className={`absolute inset-0 flex items-center justify-center bg-black/20 transition-opacity duration-300 ${!isPlaying ? 'opacity-100' : (showControls ? 'opacity-100' : 'opacity-0')}`}
                    onClick={togglePlay}
                >
